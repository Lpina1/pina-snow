  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // optional: load profile for role display
        try {
        ...
        <header style={ui.header}>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={ui.badge(status)}>
              {status === "checking" ? "Checking session…" : status === "in" ? "Signed in" : "Signed out"}
            </div>

            {status === "in" ? (
              <div style={{ marginTop: 6 }}>
                {email ? <div style={{ fontSize: 12, opacity: 0.8 }}>{email}</div> : null}
                {me ? <div style={{ fontSize: 12, opacity: 0.75 }}>role: {me.role || "unknown"}</div> : null}
              </div>
            ) : null}
          </div>
        </header>
